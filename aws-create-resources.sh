...
KEY_NAME=$(aws ec2 import-key-pair --key-name "Wireguard" --public-key-material fileb://<(cat ~/.ssh/id_rsa.pub) | jq -r '.KeyName')

echo "Starting instance"
AMI_ID=$(aws ec2 describe-images --owners amazon --filters "Name=root-device-type,Values=ebs" "Name=state,Values=available" "Name=ena-support,Values=true" "Name=virtualization-type,Values=hvm" "Name=architecture,Values=arm64" "Name=root-device-type,Values=ebs" "Name=name,Values=al*-ami-ecs-hvm-*-arm64" --query 'Images | sort_by(@, &CreationDate) | [-1].ImageId' --output text)
